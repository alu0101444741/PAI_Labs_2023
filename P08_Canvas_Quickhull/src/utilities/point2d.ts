...
export class Point2D {
  ...
  * @return distance
  */
  public distanceFromLine(line: Line2D): number {
    let coefficientA: number = line.getStartPoint().getCoordinateY() - line.getEndPoint().getCoordinateY();
    let coefficientB: number = line.getEndPoint().getCoordinateX() - line.getStartPoint().getCoordinateX();
    let coefficientC: number = line.getStartPoint().getCoordinateX() * line.getEndPoint().getCoordinateY();
        coefficientC -= line.getEndPoint().getCoordinateX() * line.getStartPoint().getCoordinateY();
    let numerator: number = Math.abs(coefficientA * this.coordinateX + coefficientB * this.coordinateY + coefficientC);
    let denominator: number = Math.sqrt(coefficientA * coefficientA + coefficientB * coefficientB);
