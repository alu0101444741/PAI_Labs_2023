...
export class ConvexhullAnimation extends CanvasView {
    ...
    */
    constructor(pointsAmount, MILISECONDS) {
        ...
        this.allPoints = this.allPoints.sort(function (firstPoint, secondPoint) {
    ...
    computeConvexHull() {
        let pointToCheck = this.allPoints[this.currentIndex];
        let vectorToNextPoint = this.nextPoint.subtract(this.currentPoint).toArray();
        if (pointToCheck !== undefined) {
            let vectorToCheckingPoint = pointToCheck.subtract(this.currentPoint).toArray();
