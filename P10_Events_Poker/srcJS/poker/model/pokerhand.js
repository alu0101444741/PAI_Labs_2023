...
export class PokerHand extends Hand {
    ...
    */
    compare(hand) {
        if (hand.label === this.label) {
            ...
            if (this.label === PokerHand.POKER_HANDS[3])
                return (this.getHighestValueCard().compare(hand.getHighestValueCard()));
            if (this.label === PokerHand.POKER_HANDS[2])
                return (this.compareSameLabel(hand, 3));
            if (this.label === PokerHand.POKER_HANDS[1]) {
                let firstValue, secondValue;
                let firstPairValue = this.firstRepeatedValue(2);
    ...
    compareSameLabel(hand, appearances) {
        let valueOne = this.firstRepeatedValue(appearances);
        let valueTwo = hand.firstRepeatedValue(appearances);
        if (valueOne === 1)
            return (true);
        if (valueTwo === 1)
            return (false);
     ...
     */
    hasFullHouse() {
        let repeatedValue = this.firstRepeatedValue(3);
        if (repeatedValue === 0)
            return (false);
     ...
     */
    hasTwoPair() {
        let repeatedValue = this.firstRepeatedValue(2);
        if (repeatedValue === 0)
            return (false);
        return (this.firstRepeatedValue(2, repeatedValue) !== 0);
    }
     ...
     */
    firstRepeatedValue(appearences, valueToIgnore) {
        let cardCount = 0;
