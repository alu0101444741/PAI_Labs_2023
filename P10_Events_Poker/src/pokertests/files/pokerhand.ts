...
export class PokerHand extends Hand {
  private static readonly POKER_HANDS: string[] = ['Pair', 'Two Pair', 'Three of a Kind', 'Straight', 'Flush', 'Full House', 'Four of a Kind', 'Straight Flush'];
  private static readonly CARDS_PER_SUIT: number = 13;
  /**
  ...
  */
  public compare(hand: PokerHand): boolean {
    if (hand.label === this.label) {
      ...
      if (this.label === PokerHand.POKER_HANDS[3]) return (this.getHighestCard().compare(hand.getHighestCard()));
      ...
      if (this.label === PokerHand.POKER_HANDS[1]) {
        ...
        let firstPairValue: number = this.hasRepeatedValues(2);
  ...
  private compareSameLabel(hand: PokerHand, appearances: number): boolean {
    let valueOne: number = this.hasRepeatedValues(appearances);
    let valueTwo: number = hand.hasRepeatedValues(appearances);
  ...
  private hasFullHouse(): boolean {
    let repeatedValue: number = this.hasRepeatedValues(3);
  ...
  private hasTwoPair(): boolean {
    let repeatedValue: number = this.hasRepeatedValues(2);
    if (repeatedValue === 0) return(false);
    return(this.hasRepeatedValues(2, repeatedValue) !== 0);
  }
   ...
   */
  private hasRepeatedValues(appearences: number, valueToIgnore?: number): number {
